...
function coffeeList( $coffee_id, $coffee_Token, $venues, $sponsors )
{
	...
	foreach ( $result as $item )
	{
		...
		$cMod -> eStart = $date['start'] -> format("d/m/Y H:i");
		$cMod -> endT = $date['end'] -> format("H:i");
		$cMod -> eEnd = $date['end'] -> format("d/m/Y H:i");
...
$client_id = "coffee-mornings-2eor6z";
$client_token = "H9TXoRnDpI9kOzj0dbMlHzjQDWZKXOS8thP1uA6MAt2lUlLuYFtll3y6ygU9Y5Uf";
$coffee_id = 11599738;
...
{
?>
	<div class='cal-event large-12 columns'>
<?php
	$d = $espresso->day;
	$st = $espresso->startT;
	$et = $espresso->endT;
	echo "<span class='large-6 medium-12 columns'>" . $d . "</span>\n";
	echo "<span class='large-6 medium-12 columns'>" . $st . " to " . $et . "</span>\n"; 
?>
	</div>
	<div class='cal-details large-12 columns'>
<?php	
	if(isset($espresso->venue[0]))
	{
		$v = $espresso->venue[0]->name;
		$a = $espresso->venue[0]->address;
		$a = str_replace("GU1, UK", "", $a);
		echo "<span class='large-12 columns '><strong>Venue:</strong> " . $v . "</span>\n<br/>\n";
		echo "<span class='large-12 columns '><strong>Address:</strong> " . $a . "</span>\n<br/>\n";
	}

	if(isset($espresso->sponsor[0]))
	{
		$g = $espresso->sponsor[0]->name;
		echo "<span class='large-12 columns '><strong>Guest Event:</strong> " . $g . "</span>\n<br/>\n"; 
	}
?>	
		<div class="cal-add">
			<div title="Add to Calendar" class="addthisevent">
			    Add to Calendar
<?php
			    echo '<span class="start">' . $espresso->eStart . '</span>';
			    echo '<span class="end">' . $espresso->eEnd . '</span>';
			    echo '<span class="timezone">Europe/London</span>';
			    echo '<span class="title">Positive Touch Coffee Morning</span>';
			    echo '<span class="description">Monthly coffee morning</span>';
			    echo '<span class="location">' . $v . '</span>';
			    echo '<span class="date_format">DD/MM/YYYY</span>';
?>
			</div>
		</div>
	</div>
